...
function ElementBadge({ element, size = 'md' }: { element?: string; size?: 'sm' | 'md' }) {
  const colors: Record<string, string> = {
    earth: 'bg-amber-700/20 text-amber-400 border-amber-700/30',
    water: 'bg-blue-700/20 text-blue-400 border-blue-700/30',
    fire: 'bg-red-700/20 text-red-400 border-red-700/30',
    air: 'bg-slate-700/20 text-slate-300 border-slate-700/30',
  }
  const sizeClasses = size === 'sm' ? 'px-2 py-0.5 text-xs' : 'px-3 py-1 text-sm'
  return (
    <span className={`rounded-full border ${colors[element || 'air']} ${sizeClasses} capitalize`}>
      {element || 'air'}
    </span>
  )
}
...
function ChakraBadge({ chakra, size = 'md' }: { chakra?: string; size?: 'sm' | 'md' }) {
  const colors: Record<string, string> = {
    root: 'bg-red-900/20 text-red-400 border-red-900/30',
    sacral: 'bg-orange-900/20 text-orange-400 border-orange-900/30',
    'solar-plexus': 'bg-yellow-900/20 text-yellow-400 border-yellow-900/30',
    heart: 'bg-green-900/20 text-green-400 border-green-900/30',
    throat: 'bg-cyan-900/20 text-cyan-400 border-cyan-900/30',
    'third-eye': 'bg-indigo-900/20 text-indigo-400 border-indigo-900/30',
    crown: 'bg-purple-900/20 text-purple-400 border-purple-900/30',
  }
  const sizeClasses = size === 'sm' ? 'px-2 py-0.5 text-xs' : 'px-3 py-1 text-sm'
  return (
    <span className={`rounded-full border ${colors[chakra || 'crown']} ${sizeClasses} capitalize`}>
      {chakra?.replace('-', ' ') || 'crown'}
    </span>
  )
}

export function OilPageClient({ slug, variant }: OilPageClientProps) {
  // Get oil data
  const oilData = useMemo(() => {
    return getOilByHandle(slug) || getOilById(slug)
  }, [slug])

  // State - crystal and cord are now managed by ProductConfigurator
  const [configuration, setConfiguration] = useState<{
    ...
    selectedCord: { id: 'waxed-cotton', name: 'Waxed Cotton Cord', price: 2.00 },
    price: 0,
    breakdown: null,
    isValid: false,
    validationMessage: 'Please select a crystal to continue',
  })

  // Get all prices for this oil
  ...
  return (
    <div className="min-h-screen bg-[#0a080c]">
      {/* Hero Section - Safe padding from header (h-20 = 80px) */}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16">
              ...
              {oilData.crystalPairings.length > 0 && (
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {oilData.crystalPairings.map((pairing) => (
                      <button
                        key={pairing.id}
                        onClick={() => setConfiguration(prev => ({ ...prev, selectedCrystal: pairing }))}
            ...
            <div className="space-y-6">
              ...
              {/* Back to Collection - Violet to stand out */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <ProductConfigurator
                  ...
                  onConfigurationChange={(config) => setConfiguration(prev => ({ ...prev, ...config }))}
                />
              </motion.div>
